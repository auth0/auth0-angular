...
async function run() {
  try {
    ...
    if (!prTitle) {
      core.setFailed(
        '⛔️ Could not get pull request title. This action should only be run on a pull_request event.'
      );
      return;
    }
    ...
    if (result.valid) {
      ...
      core.info(details);
      core.setFailed(
        `PR title "${prTitle}" does not follow the Conventional Commits format.`
      );
    }
  } catch (error) {
